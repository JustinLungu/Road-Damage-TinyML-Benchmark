...
from src.constants import EFFICIENTFORMER_MODEL_IDS  # noqa: E402
...
SUPPORTED_MODEL_NAMES = tuple(EFFICIENTFORMER_MODEL_IDS)
...
def validate_config() -> None:
    ...
    unknown_model_names = [
        ...
        if model_name not in EFFICIENTFORMER_MODEL_IDS
    ]
...
def get_imagenet_labels() -> list[str]:
    """Return human-readable ImageNet-1K labels in class-index order."""
    imagenet_info = ImageNetInfo()
    return [
        imagenet_info.index_to_description(class_id)
        for class_id in range(1000)
    ]


def tensor_shape(tensor: torch.Tensor) -> tuple[int, ...]:
    """Record tensor shape without saving tensor values to JSON."""
    return tuple(tensor.shape)


def get_model_data_config(model: torch.nn.Module) -> dict[str, Any]:
    """Ask timm which preprocessing settings belong to this checkpoint."""
...
def save_results_json(
    ...
    output = {
        ...
        "timm_model_id": EFFICIENTFORMER_MODEL_IDS[model_name],
...
def run_model(
    ...
    data_config = get_model_data_config(model)
    transform = timm.data.create_transform(**data_config, is_training=False)

    # timm resolves the checkpoint's resize, crop, normalization, and tensor
    # layout through data_config.
    image = load_rgb_image(image_path)
    ...
    rows = topk_rows(logits, labels)
    input_shape = tensor_shape(input_tensor)
    print(f"Model: {model_name}")
    print(f"timm model ID: {EFFICIENTFORMER_MODEL_IDS[model_name]}")
    ...
    if SAVE_RESULTS_JSON:
        save_results_json(model_name, image_path, rows, input_shape, data_config)
...
def main() -> None:
    ...
    for model_name in MODEL_NAMES:
        run_model(model_name, image_path, device, labels)
