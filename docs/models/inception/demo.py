...
from src.constants import INCEPTION_MODEL_CHECKPOINTS  # noqa: E402
...
SUPPORTED_MODEL_NAMES = tuple(INCEPTION_MODEL_CHECKPOINTS)
...
def save_results_json(
    ...
    output = {
        ...
        "checkpoint": INCEPTION_MODEL_CHECKPOINTS[model_name],
...
def run_model(model_name: str, image_path: Path, device: torch.device) -> None:
    ...
    # Torchvision weights define the exact 299x299 crop and normalization
    # expected by the pretrained checkpoint.
    image = load_rgb_image(image_path)
    input_tensor = transform(image).unsqueeze(0).to(device)

    with torch.inference_mode():
        # Evaluation mode disables the training-only auxiliary logits output.
